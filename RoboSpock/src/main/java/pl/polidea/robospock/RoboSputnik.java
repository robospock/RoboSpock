...
public class RoboSputnik extends Runner implements Filterable, Sortable {
    ...
    // we're using interface, because using Sputnik by class would throw "cannot cast from Sputnik to Sputnik"
    private Runner sputnikRunner;

    public RoboSputnik(Class<?> clazz) throws InitializationError {

        RobolectricClassLoader classLoader = createClassLoader();

        // this line prevents overloading class loader ? LOL
        classLoader.delegateLoadingOf(ArrayUtil.class.getName());

        Class<?> delegateClass = classLoader.bootstrap(Sputnik.class);
        try {
            // that's the only reason for new runner - to delegate to sputnik, which is loaded with Robolectric classloader
    ...
    private static RobolectricClassLoader createClassLoader() {
        ...
        RobolectricClassLoader classLoader = new RobolectricClassLoader(ShadowWrangler.getInstance());
        Thread.currentThread().setContextClassLoader(classLoader);

        // those classes are not instrumented for sure and they instrumenting breaks things
